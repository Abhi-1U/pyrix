...
class BinaryMatrix(Matrix):
    ...
    def __mul__(self, BinaryMat2):
        print(
            "Multiplication on binary Matrices will Change the complete structure and make it impossibly wierd to represent."
        )
    ...
    def __and__(self, m2):
        ...
        if self.matrix.binaryMatrix == True and m2.matrix.binaryMatrix == True:
            if self.matrix.dimensions == m2.matrix.dimensions:
                ...
                return BinaryMatrix(
                    nrow=self.matrix.nrow, ncol=self.matrix.ncol, data=data
                )
            else:
                raise incompaitableTypeException
        else:
    ...
    def __invert__(self):
        ...
        if self.matrix.binaryMatrix == True:
            ...
            for i in range(self.matrix.nrow):
                data.append([])
                for j in range(self.matrix.ncol):
                    data[i].append(__Not(self.matrix.data[i][j]))
    ...
    def twosComplement(self):
        ...
        if lastelement == 0:
            ...
            data = self.__forward_one(
                data=binaryinvertedmatrix.matrix.data,
                rowcount=lastrow,
                colcount=lastcol,
            )
